...
# dat = readRDS(file = "data/landreth_fishmacros_data.rds")
model_list = readRDS(file = "models/model_list.rds")
dat = as_tibble(model_list[[2]]$data)
...
epred_draws = dat_resampled_rank %>% 
  ...
  distinct() %>% 
  mutate(counts = 1) %>% 
  add_epred_draws(model_list[[2]], re_formula = NULL)
...
ggsave(plot_isd, width = 6.5, height = 7,
       file = paste0(directory, ".jpg"), dpi = 300)
saveRDS(plot_isd, file = paste0(directory, ".rds"))
