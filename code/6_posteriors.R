...
# wrangle and plot ----------------
post_lines_group = post_lines %>% filter(!is.na(name)) %>% group_split(model)
post_dots_group = post_dots %>% filter(!is.na(name)) %>% group_split(model)
...
palette_colors <- brewer.pal(n = length(post_lines_group), name = "Set2")

palette_colors[[2]] = "#FC8D62"
palette_colors[[3]] = "#FC8D62"
palette_colors[[4]] = "#FC8D62"

for(i in seq_along(post_lines_group)){
  post_plots[[i]] = post_lines_group[[i]] %>%  
    ggplot(aes(x = effect1__, y = estimate__)) + 
    geom_line() +
    geom_ribbon(aes(ymin = lower__, ymax = upper__, fill = model, color = model), alpha = 0.3) +
    ggh4x::facet_grid2(name ~ model) + 
    stat_pointinterval(data = post_dots_group[[i]], aes(x = value, 
                                                        y = .epred, group = stream, 
                                                        color = model), size = 0.2) +
    scale_color_manual(values = palette_colors[[i]]) +  # Apply custom colors
    scale_fill_manual(values = palette_colors[[i]]) +  # Apply custom colors
    guides(fill = "none",
           color = "none") +
    labs(y = "\u03bb",
...
panel_posts = (post_plots[[1]] + post_plots[[5]] + post_plots[[6]])/(post_plots[[2]] + post_plots[[3]] + post_plots[[4]])

ggsave(panel_posts, file = "plots/panel_posts.jpg", width = 10, height = 7, dpi = 500)
...
post_dots %>%
  ...
  mutate(median = median(.epred), 
         name = as.factor(name),
         name = fct_relevel(name, "intercept_s")) %>% 
  ggplot(aes(y = reorder(stream, median), 
             x = .epred, color = interaction(name))) +
