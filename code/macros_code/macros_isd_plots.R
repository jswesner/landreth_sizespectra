...
directory = "plots/macros_plots/"

#1) Load models and data
# load data and models -----------------------------------------
model_files_macros = list.files(path = "models/macros_models", pattern = "\\.rds$", full.names = T)
model_lists_macros <- list()

for (file in model_files_macros) {
  model_list_macros[[file]] <- readRDS(file)
}

dat = as_tibble(model_list_macros[[2]]$data)
...
epred_draws = dat_resampled_rank %>% 
  ...
  distinct() %>% 
  mutate(counts = 1) %>% 
  add_epred_draws(model_list_macros[[2]], re_formula = NULL)
...
ggsave(plot_isd, width = 6.5, height = 7,
       file = paste0(directory, "macros_plot_isd.jpg"), dpi = 300)
saveRDS(plot_isd, file = paste0(directory, "macros_plot_isd.rds"))
